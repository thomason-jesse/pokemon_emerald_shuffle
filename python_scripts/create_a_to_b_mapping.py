...
def get_a_to_b_penalty(a, b, mon_map, mon_metadata, mon_evolution, type_map,
    ...
    for type_str, weight in [["type1", 0.67], ["type2", 0.33]]:
        if mon_metadata[a][type_str] not in type_map:
            missing_from_map += 1
        elif (mon_metadata[b][type_str] != type_map[mon_metadata[a][type_str]] or # Penalize for mismatch in mapping.
...
def score_map(mon_map, mon_list, type_list, mon_metadata, mon_evolution, penalize_same_type,  mon_n_appearances,
              as_exp_str=False):
...
def perform_assignment_given_type_map(n_mon, mon_list, mon_map, mon_image,
                                      mon_metadata, mon_evolution, mon_evolution_r,
    ...
    relevant_mon = [a for a in mon_list if mon_metadata[a]['type1'] in type_map]
...
def main(args):
    ...
    # If we're optimizing the type map itself, need to do a tedious, greedy beam search for each type.
        ...
        type_appearances = [sum([mon_n_appearances[mon_list.index(a)]
                                 if (mon_metadata[a]["type1"] == t or mon_metadata[a]["type2"] == t)
                                 else 0
